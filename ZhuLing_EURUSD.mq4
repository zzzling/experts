...
int start()
  ...
  //check if we should place order
   ...
   double MACDMain = iMACD(CurrentSymbol,PERIOD_M15,12,26,9,PRICE_CLOSE,MODE_MAIN,0);
   double MACDSignal = iMACD(CurrentSymbol,PERIOD_M15,12,26,9,PRICE_CLOSE,MODE_SIGNAL,0);   

   double MACDMainP1 = iMACD(CurrentSymbol,PERIOD_M15,12,26,9,PRICE_CLOSE,MODE_MAIN,1);
   double MACDSignalP1 = iMACD(CurrentSymbol,PERIOD_M15,12,26,9,PRICE_CLOSE,MODE_SIGNAL,1);   
   ...
   //check MA indicator
   double Ma5 = iMA(CurrentSymbol,PERIOD_M15,5,0,MODE_SMMA,PRICE_CLOSE,0);
   double Ma5Pre = iMA(CurrentSymbol,PERIOD_M15,5,0,MODE_SMMA,PRICE_CLOSE,1);
   double Ma5Pre2 = iMA(CurrentSymbol,PERIOD_M15,5,0,MODE_SMMA,PRICE_CLOSE,2);
   
   double Ma8 = iMA(CurrentSymbol,PERIOD_M15,8,0,MODE_SMMA,PRICE_CLOSE,0);
   double Ma8Pre = iMA(CurrentSymbol,PERIOD_M15,8,0,MODE_SMMA,PRICE_CLOSE,1);
   double Ma8Pre2 = iMA(CurrentSymbol,PERIOD_M15,8,0,MODE_SMMA,PRICE_CLOSE,2);

   double Ma26 = iMA(CurrentSymbol,PERIOD_M15,26,0,MODE_SMMA,PRICE_CLOSE,0);
   double Ma26Pre = iMA(CurrentSymbol,PERIOD_M15,26,0,MODE_SMMA,PRICE_CLOSE,1);
   double Ma26Pre2 = iMA(CurrentSymbol,PERIOD_M15,26,0,MODE_SMMA,PRICE_CLOSE,2);
...
//   
   ...
   {
      double smallMA = iMA(CurrentSymbol,PERIOD_M15,5,0,MODE_SMA,PRICE_MEDIAN,i);
      double bigMA = iMA(CurrentSymbol,PERIOD_M15,34,0,MODE_SMA,PRICE_MEDIAN,i);
...
//         DrawSignal(5,Red);       
   ...
   //check KDJ indicator;
   //---------------------------------------------------------------------------------
   double K = iStochastic(CurrentSymbol,PERIOD_M15,5,3,3,MODE_SMA,0,MODE_MAIN,0);
   double D = iStochastic(CurrentSymbol,PERIOD_M15,5,3,3,MODE_SMA,0,MODE_SIGNAL,0);
   double Kp1 = iStochastic(CurrentSymbol,PERIOD_M15,5,3,3,MODE_SMA,0,MODE_MAIN,1);
   double Kp2 = iStochastic(CurrentSymbol,PERIOD_M15,5,3,3,MODE_SMA,0,MODE_MAIN,2);
   double Dp1 = iStochastic(CurrentSymbol,PERIOD_M15,5,3,3,MODE_SMA,0,MODE_SIGNAL,1);
   double Dp2 = iStochastic(CurrentSymbol,PERIOD_M15,5,3,3,MODE_SMA,0,MODE_SIGNAL,2);
   //check buy
   //check K>D
   if((K > D) && (Kp1 < Dp1))
   {
   ...
   if(CheckNoTradeSignal(CurrentSymbol))
   {
      return;
   }
...
bool CheckNoTradeSignal(string CurrentSymbol)
{
   return (CheckGap(CurrentSymbol)|| CheckTiming(CurrentSymbol) || CheckHugePriceChange(CurrentSymbol));
}

//如果是每个月第一个周五的晚上8点到11点，非农数据公布的时候，不开单
bool CheckTiming(string CurrentSymbol)
{
   //周五晚上7点以后不交易
   if(DayOfWeek() == 5)
   {
...
bool CheckGap(string CurrentSymbol)
{
   static datetime LastGapTime = 0;

   double PriceThresold = 50*PointScale( CurrentSymbol)*MarketInfo(CurrentSymbol,MODE_POINT);

   if( MathMax(iOpen(CurrentSymbol,PERIOD_M15,1),iClose(CurrentSymbol,PERIOD_M15,1)) < MathMin(iOpen(CurrentSymbol,PERIOD_M15,0),iClose(CurrentSymbol,PERIOD_M15,0)) - PriceThresold
      ||
      MathMin(iOpen(CurrentSymbol,PERIOD_M15,1),iClose(CurrentSymbol,PERIOD_M15,1)) > MathMax(iOpen(CurrentSymbol,PERIOD_M15,0),iClose(CurrentSymbol,PERIOD_M15,0)) + PriceThresold)
   {   
      return (true);
   }
   else
...
bool CheckHugePriceChange(string CurrentSymbol)
{
   static datetime LastOcilationTime = 0;
   
   double PriceDifference = MathAbs(iOpen(CurrentSymbol,PERIOD_M15,0) - iClose(CurrentSymbol,PERIOD_M15,0));
   double PriceThresold = 50*PointScale( CurrentSymbol)*MarketInfo(CurrentSymbol,MODE_POINT);
