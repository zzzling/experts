...
int start()
  {
   ...
   int IntegerPrice = price/Point;
   Print(IntegerPrice);
   
   int LastThreePoints = IntegerPrice - (IntegerPrice/1000)*1000;
  
   double upper = ((IntegerPrice/1000)*1000 + LastThreePoints * 1.382) *Point;
   
   double lower = ((IntegerPrice/1000)*1000 + LastThreePoints * 0.618) *Point;

   
   int ticket;
   ticket = OrderSend(Symbol(), OP_BUY, 1, Ask, 5, upper, lower, "Test Oreder 1", 0, 0, Green);
   if(ticket<0)
   {
      Print("OrderSend 失败 ",ErrorDescription (GetLastError()));
   }
